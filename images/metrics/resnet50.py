...
SWAV_PRETRAINED_WEIGHTS = './metrics/swav_800ep_pretrain.pth.tar' # SWAV_PRETRAINED_WEIGHTS_URL = 'https://www.dropbox.com/s/ghjrgktg0lyjorn/swav_800ep_pretrain.pth.tar'
# This has to be the same widely available pth (pre-trained on imagenet)
# SWAV_PRETRAINED_WEIGHTS_URL = 'https://dl.fbaipublicfiles.com/deepcluster/swav_800ep_pretrain.pth.tar'
...
def resnet50(pretrained_mode, **kwargs):
    resnet = ResNet(Bottleneck, [3, 4, 6, 3], **kwargs)


    state_dict = torch.load(SWAV_PRETRAINED_WEIGHTS, map_location='cuda' if torch.cuda.is_available() else 'cpu')
