...
# Helper function to add a border to an image
def add_border_to_image(image, border_color, border_width):
    """
    Adds a border of a specified color and width to an image.
    
    Args:
        image (torch.Tensor): The image tensor of shape (C, H, W).
        border_color (list): The RGB color of the border, e.g., [1.0, 0.0, 0.0] for red.
        border_width (int): The width of the border in pixels.
    
    Returns:
        torch.Tensor: The image with the border added.
    """
    bordered_image = image.clone()
    device = image.device
    
    # Set the border pixels to the specified color
    bordered_image[:, :border_width, :] = torch.tensor(border_color).view(3, 1, 1).to(device)  # Top border
    bordered_image[:, -border_width:, :] = torch.tensor(border_color).view(3, 1, 1).to(device)  # Bottom border
    bordered_image[:, :, :border_width] = torch.tensor(border_color).view(3, 1, 1).to(device)  # Left border
    bordered_image[:, :, -border_width:] = torch.tensor(border_color).view(3, 1, 1).to(device)  # Right border
...
def train_introspective_vae(dataset='cifar10', batch_size=128, num_workers=0, 
    ...
    for epoch in range(0, num_epochs):
        ...
        for iter_index, (batch, batch_labels) in enumerate(pbar):
                    ...
                    if epoch < num_vae: 
                        ...
                        soft_assignment_real = kl_soft_assignments(model, z_s=z_kl_hook,
                                                                        target_mu=mu_MoG, 
                        ...
                        fid_inception_train_log_list.append(fid_inception_train.item())
                        assignment_log_list.append(assignment_neg_entropy.item())
                        ...
                        if fid_inception_train_log_list[-1] != fid_inception_train_log_list[-2]:
                            ## plotting fake images and qualitative
                            extract_qualitative(model, train_set, save_dir=result_dir, beta_rec=beta_rec, beta_kl=beta_kl, MC=MC, recon_loss_type=recon_loss_type,
                                kl_loss_type=kl_loss_type, batch_size=batch_size, it=epoch, nrow=num_row, device=device, figures_to_include=figures_to_include, 
                            ...
                            noise_batch, noise_indices = model.sample_noise(18, ret_ind=True, with_grad=sampling_with_grad)
                            fake_img = model.sample(noise_batch)
                            # Get the assignment values for the noise indices
                            noise_indices_assignment = soft_assignment_real.mean(axis=1)[noise_indices]
                            
                            for i in range(fake_img.shape[0]):

                                if noise_indices_assignment[i] > 1/(num_components ** 2):
                                    border_color = [0.0, 0.0, 0.0]  # Black border
                                else:
                                    border_color = [1.0, 0.0, 0.0] 

                                fake_img[i] = add_border_to_image(fake_img[i], border_color=border_color, border_width=2)

                            # Save the resulting image with red borders
                            vutils.save_image(fake_img,
                                            os.path.join(result_dir, 'bordered_fake_img_{}.png'.format(epoch)),
                                            nrow=6)
    ...
    with open(os.path.join(result_dir,'assignment_log_list.txt'), 'w') as f:
        for item in assignment_log_list:
            f.write("%s\n" % item)
